...
public class Stem {
    ...
    //--------------------------------------------------------------------------
    // constructor
    public Stem(File fileToBeStemmed, Vector statFiles) {
        ...
        for (int i = 0; i < wordNumber; i++) {
            all.append(stemmedDocument[i][0] + "\t" + stemmedDocument[i][1] + "\t" + stemmedDocument[i][2] + "\n");
        }
    }

    public Stem() {
        String pathToStemmerFiles = new StringBuffer(System.getProperty("user.dir") + System.getProperty("file.separator") + "\\src\\assets\\StemmerFiles" + System.getProperty("file.separator")).toString();
    ...
    public String StemWord(String word) {
        ...
        if (removeDiacritics(word, modifiedWord)) {
            sword = modifiedWord.toString();
        }

        // remove any punctuation from the word
        if (removePunctuation(sword, modifiedWord)) {
            sword = modifiedWord.toString();
        }

        // there could also be characters that aren't letters which should be removed
        if (removeNonLetter(sword, modifiedWord)) {
            sword = modifiedWord.toString();
        }

        // check for stopwords
        if (!checkStrangeWords(sword)) // check for stopwords
        {
            if (!checkStopwords(sword)) {
                sword = stemWord(sword);
            }
        }
    ...
    private boolean removeDiacritics(String currentWord, StringBuffer modifiedWord) {
        ...
        for (int i = 0; i < currentWord.length(); i++) // if the character is not a diacritic, append it to modified word
        {
